...
Umax = zeros(100,1);
% maRange = linspace(1,113);
tiltAngle = linspace(0/180*pi,20/180*pi);
for j = 1:100
%% system parameters
...
x1(1,1) = tiltAngle(j);         % upper body angle (rad), 20deg
...
plot(tiltAngle/pi*180,Umax);
xlabel('Initial Tilt Angle (deg)')
ylabel('Maximum Required Torque to Recover (Nm)')

%xlabel('Upper Body Mass (kg)')
%ylabel('Maximum Required Torque to recover from 20deg (Nm)')
