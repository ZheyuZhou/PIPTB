syms phi(t) psi(t) omg(t) mk mw mu rk rw L D tau(t)
...
TAWL = 1/2*mw*((rk*dphi)^2 + 2*(rk+rw)*cos(omg(t))*domg*(rk*dphi) + (rk+rw)^2*domg^2) + 1/2*IW*(rk/rw*(dphi-domg)-domg)^2;
TUBL = 1/2*mu*((rk*dphi)^2 + 2*L*cos(omg(t))*domg*(rk*dphi) + L^2*domg^2) + 1/2*IU*(domg)^2;
...
TAWP = 1/2*mw*((rk*dphi)^2 + 2*(rk+rw)*cos(omg(t))*domg*(rk*dphi) + (rk+rw)^2*domg^2) + 1/2*IW*(rk/rw*(dphi-domg)-domg)^2 + 1/2*mw*(dphi*rk/D)^2;
TUBP = 1/2*mu*((rk*dphi)^2 + 2*L*cos(omg(t))*domg*(rk*dphi) + L^2*domg^2) + 1/2*IU*(domg)^2 + 1/2*mu*(dphi*rk/D)^2;
